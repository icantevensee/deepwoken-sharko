from        PyQt5.QtWidgets import QLabel, QStyleOptionMenuItem, QProxyStyle, QStyle, QWidget, QTextEdit, QFileIconProvider, QHBoxLayout, QVBoxLayout, QMainWindow
from        PyQt5.QtGui import QColor, QFont, QPainter, QCursor, QPen, QTextCursor, QPalette
from        PyQt5.QtCore import QTimer, Qt, QRect, QFileInfo, QPoint, QEvent,pyqtSignal 
            ...
            if isinstance(option, QStyleOptionMenuItem):
                ...
                if option.checkType != QStyleOptionMenuItem.NotCheckable:
                    ...
                    if option.checked:
                        ...
                        check_color = QColor("#808080") if not is_enabled else QColor("#D5FFFF")
                ...
                if option.menuItemType == QStyleOptionMenuItem.SubMenu:
                    ...
                    arrow_color = QColor("#808080") if not is_enabled else QColor("#D5FFFF")
...
class SwordWindow(QWidget):
    ...
    def attack(self):
        ...
        if self.is_attacking or self.is_appearing: return
    ...
    def engine_loop(self):
        ...
        if self.is_attacking and not self.is_appearing:
            ...
            if self.damage_callback is not None:
                if not hasattr(self, "_damage_fired") or not self._damage_fired:
                    if p >= 0.5:
                        self.damage_callback()
                        print("Damage callback fired!")
                        self._damage_fired = True
            else:
                # Reset flag if no callback is configured
                self._damage_fired = False
    ...
    def lerp(self, start, end, t): return start + (end - start) * t
    def ease_in_out(self, t): return t * t * (3 - 2 * t)
    def ease_out_back(self, t): 
        c1, c3 = 1.70158, 2.70158
        return 1 + c3 * math.pow(t - 1, 3) + c1 * math.pow(t - 1, 2)
    def ease_out_expo(self, t): return 1 if t == 1 else 1 - math.pow(2, -10 * t)
...
class FakeCommandPrompt(QMainWindow):
    ...
    def keyPressEvent(self, event): event.ignore()
    def keyReleaseEvent(self, event): event.ignore()
